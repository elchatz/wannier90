...
contains
  ...
  !================================================!
  subroutine w90_wannier90_readwrite_read_special(settings, atom_data, band_plot, dis_control, dis_spheres, &
...
!    endif
  ...
  subroutine w90_wannier90_readwrite_read_projections(settings, proj, use_bloch_phases, lhasproj, &
                                                      ...
                                                      atom_data, recip_lattice, num_bands, &
                                                      num_wann, gamma_only, spinors, bohr, stdout, &
                                                      error, comm)
    !================================================!
    use w90_error, only: w90_error_type
    ...
    if (found) then
      ...
      call w90_readwrite_get_projections(settings, num_proj, atom_data, num_bands, num_wann, &
                                         proj_input, recip_lattice, .true., spinors, bohr, stdout, &
                                         error, comm)
      if (allocated(error)) return
      lhasproj = .true.
    ...
    if (lhasproj) then
      ...
      call w90_readwrite_get_projections(settings, num_proj, atom_data, num_bands, num_wann, &
                                         proj_input, recip_lattice, .false., spinors, bohr, &
                                         stdout, error, comm)
      if (allocated(error)) return
